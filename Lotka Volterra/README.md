...
**Note:** the log-space integrator (`rk4_step_log` / `integrate_forward_log`) is used to generate the ground-truth trajectory data. The Neural ODE's own forward integration during training uses the classical (linear-space) RK4 integrator, since the network output is not itself constrained to be a log-derivative — see Possible Improvements below.
...
Training is run over a fixed **3.5 second horizon** ($t \in [0, 3.5]$, $\Delta t = 0.01$). Longer horizons were found to produce a severe loss plateau regardless of architecture or learning rate — training on the shorter horizon directly avoids this rather than solving it, and is the current working approach.
...
| **Training Execution**      | Pass   | Final Loss $\approx 6.8 \times 10^{-3}$  | Stabilizes within 5000 epochs on a 3.5s horizon, 0.01 learning rate.                  |
 
---
 
## Possible Improvements
 
- **Longer-Horizon Training:** The current approach trains on a short (3.5s) fixed horizon to avoid a loss plateau observed on longer horizons. Resolving this properly — via mini-batching, sub-sequencing, or staged curriculum training on progressively longer horizons — would let the model learn the full oscillatory cycle rather than a truncated window of it.
...
- **NODE-as-Solver Mode:** The architecture includes a `:solver` mode where the known Lotka-Volterra equations serve as the right-hand side and the adjoint machinery acts as a learned integrator rather than a learned dynamics model. This is scaffolded in `node_architecture.jl` but not yet trained or tested.
---
