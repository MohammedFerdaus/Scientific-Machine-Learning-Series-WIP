# Project 02 — NODE: Lotka-Volterra Neural ODE
...
2. NODE — Lotka-Volterra Neural ODE ← you are here
...
The log-space integrator (`rk4_step_log` / `integrate_forward_log`) is used to generate the ground-truth trajectory data. The Neural ODE's own forward integration during training uses the classical (linear-space) RK4 integrator, since the network output is not itself constrained to be a log-derivative.
...
Training is run over a fixed 3.5 second horizon ($t \in [0, 3.5]$, $\Delta t = 0.01$).
